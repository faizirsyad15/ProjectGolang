...
import (
	...
	ep "projectHotel/hotel/kota/endpoint"
...
func main() {
	...
	var server pb.KotaServiceServer
	{
		...
		service := svc.NewKota(dbReadWriter)
		endpoint := ep.NewKotaEndpoint(service)
		fmt.Println(endpoint)
		server = ep.NewGRPCKotaServer(endpoint, tracer, logger)
	}
	...
	pb.RegisterKotaServiceServer(grpcServer, server)
