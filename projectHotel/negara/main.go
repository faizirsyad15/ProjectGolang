...
import (
	...
	ep "projectHotel/hotel/negara/endpoint"
...
func main() {
	...
	var server pb.NegaraServiceServer
	{
		...
		service := svc.NewNegara(dbReadWriter)
		endpoint := ep.NewNegaraEndpoint(service)
		fmt.Println(endpoint)
		server = ep.NewGRPCNegaraServer(endpoint, tracer, logger)
	}
	...
	pb.RegisterNegaraServiceServer(grpcServer, server)
